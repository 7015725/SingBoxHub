/* SingBoxHub Stage44 Retry1 ShortX Shell binary check. Rhino ES5 only. */
SBH.versions.shortxShellBinaryCheck = 1;
...
(function () {
    ...
    var TEMP_DIR_NAME = "ephemeral-shell-check";
    ...
    function tlsObject(query, forceEnabled) {
        ...
        if (security === "reality" || query.pbk || query.publickey) {
    ...
    function transportObject(query, fallback) {
        ...
        if (type === "ws" || type === "websocket") {
            ...
            if (query.host) {
                transport.headers = { Host: String(query.host) };
            }
            return transport;
        }
        if (type === "grpc") {
    ...
    function fromGenericUri(uriText, row) {
        ...
        var candidate = {
            ...
            server_port: numberOrNull(
                uri.getPort() >= 0 ? uri.getPort() : row.port
            )
        };
        var tls;
        var transport;
        ...
        if (type === "vless") {
            ...
            transport = transportObject(query, row.transport);
            if (tls !== null) {
                candidate.tls = tls;
            }
            if (transport !== null) {
        ...
        if (type === "hysteria2") {
            candidate.password = String(
                user.first || query.password || query.auth || ""
            );
    ...
    function fromClashProxy(proxy, row) {
        var type = normalizeType(proxy.type || row.protocol);
        var query = {};
        var candidate = {
            type: type,
            tag: stableTag(row),
            server: trim(proxy.server || row.server),
            server_port: numberOrNull(proxy.port || row.port)
        };
        var tls;
        var transport;

        query.security = proxy.security || (bool(proxy.tls) ? "tls" : "");
        query.sni = proxy.sni || proxy.servername || proxy.server_name;
        query.insecure = proxy.skip_cert_verify || proxy["skip-cert-verify"];
        query.type = proxy.network || proxy.transport;
        query.path = proxy.path ||
            (proxy["ws-opts"] && proxy["ws-opts"].path);
        query.host = proxy.host ||
            (proxy["ws-opts"] &&
                proxy["ws-opts"].headers &&
                proxy["ws-opts"].headers.Host);
        query.servicename = proxy.service_name ||
            (proxy["grpc-opts"] &&
    ...
    function candidateFromEnvelope(envelope, row) {
        ...
        if (format === "clash_proxy") {
            return fromClashProxy(payload.proxy || {}, row);
        }
    ...
    function validateCandidate(candidate) {
        ...
        if (type === "vless" && !trim(candidate.uuid)) {
            throw new Error("UUID_MISSING");
        }
        if (type === "hysteria2" && !trim(candidate.password)) {
    ...
    function loadRows() {
        ...
        try {
            cursor = SBH.database.open().rawQuery(
                ...
                "n.protocol,n.server,n.port,n.tls,n.transport," +
            ...
            while (cursor.moveToNext()) {
                rows.push({
                    ...
                    recordKey: String(cursor.getString(8) || ""),
                    sourceFormat: String(cursor.getString(9) || "")
                });
            }
            return rows;
        } finally {
    ...
    function collectSensitive(map, candidate, envelope) {
        addSensitive(map, candidate.server);
        addSensitive(map, candidate.uuid);
        addSensitive(map, candidate.password);
        try {
            if (candidate.tls) {
                addSensitive(map, candidate.tls.server_name);
                if (candidate.tls.reality) {
                    addSensitive(map, candidate.tls.reality.public_key);
                    addSensitive(map, candidate.tls.reality.short_id);
                }
            }
            if (candidate.obfs) {
                addSensitive(map, candidate.obfs.password);
            }
            if (envelope && envelope.payload) {
                addSensitive(map, envelope.payload.uri);
            }
        } catch (ignored) {}
    }
    ...
    function buildConfig() {
        ...
        try {
            for (index = 0; index < rows.length; index += 1) {
                ...
                if (plaintext === null) {
                    throw new Error("VAULT_RECORD_MISSING");
                }
                envelope = JSON.parse(plaintext);
                candidate = validateCandidate(
                    candidateFromEnvelope(envelope, row)
                );
                collectSensitive(sensitive, candidate, envelope);
        ...
        digestParts.sort();
        return {
            config: { outbounds: candidates },
            candidateCount: candidates.length,
            typeCounts: typeCounts,
            sensitive: sensitive,
            candidateSetSha256: sha256Text(digestParts.join("\n"))
        };
    }
    ...
    function wipeDelete(file, byteCount) {
        var output = null;
        var zeros = P.java.lang.reflect.Array.newInstance(
            P.java.lang.Byte.TYPE,
            8192
        );
    ...
    function runShell(binary, configFile) {
        var command =
            "umask 077; " +
            "printf '__SBH_UID__=%s\\n' \"$(id -u 2>/dev/null)\"; " +
            "if command -v timeout >/dev/null 2>&1; then " +
            "printf '__SBH_TIMEOUT__=1\\n'; " +
            "timeout " + String(CHECK_TIMEOUT_SECONDS) + "s " +
            shellQuote(binary.getAbsolutePath()) +
            " check -c " + shellQuote(configFile.getAbsolutePath()) +
            "; else " +
            "printf '__SBH_TIMEOUT__=0\\n'; " +
            shellQuote(binary.getAbsolutePath()) +
            " check -c " + shellQuote(configFile.getAbsolutePath()) +
            "; fi";
        ...
        var code = Number(data.get("shellCode"));
        var uid = parseMarker(combined, "UID");
        var timeoutUsed = parseMarker(combined, "TIMEOUT") === 1;
    ...
    function replaceLiteral(value, needle, replacement) {
        return String(value).split(String(needle)).join(String(replacement));
    }

    function sanitize(value, tempPath, sensitive) {
        ...
        for (index = 0; index < keys.length; index += 1) {
            output = replaceLiteral(output, keys[index], "<redacted>");
        }
    ...
    function runtimeBinary() {
        ...
        if (typeof shortx === "undefined" ||
            ...
            throw new Error("SHORTX_ROOT_UNAVAILABLE");
        }
        root = new File(String(shortx.getShortXDir()), "SingBoxHub");
        return new File(new File(root, "bin"), "sing-box");
    }
    ...
    function executeCheck() {
        ...
        var binary = null;
        var tempDir = new File(SBH.paths.cacheDir, TEMP_DIR_NAME);
        var tempFile = new File(
            tempDir,
            "candidate-" + String(SBH.util.randomToken()) + ".json"
        );
        var byteCount = 0;
        ...
        try {
            built = buildConfig();
            configText = JSON.stringify(built.config, null, 2) + "\n";
            ...
            byteCount = writeUtf8Sync(tempFile, configText);
            try {
            ...
            shellResult = runShell(binary, tempFile);
            ...
            return {
                ok: false,
                stage: "credential_stage44_retry1_shortx_shell_check",
                errorCode: "SHORTX_SHELL_BINARY_CHECK_FAILED",
            ...
            cleanup = wipeDelete(tempFile, byteCount);
            try {
        ...
        return {
            ok: passed && cleanup.deleted === true,
            stage: "credential_stage44_retry1_shortx_shell_check",
            candidateCount: built.candidateCount,
            candidateTypeCounts: built.typeCounts,
            candidateSetSha256: built.candidateSetSha256,
            temporaryConfigSha256: configSha256,
            temporaryConfigByteCount: byteCount,
            temporaryConfigWritten: byteCount > 0,
            temporaryConfigLocation: "client_cache_ephemeral",
            temporaryConfigPathReturned: false,
            temporaryConfigPersisted: cleanup.deleted !== true,
            temporaryConfigOverwriteAttempted: cleanup.overwriteAttempted,
            temporaryConfigOverwriteSucceeded: cleanup.overwriteSucceeded,
            temporaryConfigDeleted: cleanup.deleted,
            temporaryConfigSecureEraseGuaranteed: false,
            executionTransport: "shortx_shell_action",
            shortxExecuteActionUsed: true,
            javaProcessBuilderUsed: false,
            shellUid: shellResult.uid,
            timeoutToolUsed: shellResult.timeoutUsed,
            singBoxBinaryCheckInvoked: true,
            singBoxCheckTimedOut: shellResult.code === 124,
            singBoxExitCode: shellResult.code,
            singBoxCheckPassed: passed,
            singBoxOutputSha256: sha256Text(shellResult.output),
            sanitizedDiagnostic: passed ? null : diagnostic,
            candidateObjectsReturned: false,
            plaintextCredentialReturned: false,
            plaintextCredentialPersisted: false,
            productionConfigModified: false,
    ...
    function binaryCheckAsync(callback) {
        if (typeof callback !== "function") {
            throw new Error("二进制检查回调不可用");
        }
        new Thread(
            new JavaAdapter(Runnable, {
                run: function () {
                    postCallback(callback, executeCheck());
                }
            }),
            "SingBoxHub-ShortXShellBinaryCheck"
        ).start();
        return {
            accepted: true,
            manualOnly: true,
    ...
    if (!vault || !coverageAudit || !service) {
        throw new Error("Stage44 Retry1 dependencies unavailable");
    }

    service.binaryCheckAsync = binaryCheckAsync;
    service.executionTransport = "shortx_shell_action";
    ...
    SBH.app.start = function () {
        ...
        output.shortxShellBinaryCheckVersion = 1;
        output.shortxShellBinaryCheckReady = true;
        output.candidateConfigPreflightVersion = 3;
        output.candidateBinaryCheckTransport =
            "shortx_shell_action";
        output.candidateBinaryCheckShortxExecuteActionUsed = true;
        ...
        output.ephemeralBinaryCheckProductionConfigModified = false;
