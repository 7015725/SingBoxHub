...
SBH.versions.runtimeArchiveInventory = 1;
(function () {
var P = Packages;
...
SBH.paths.cacheDir,
"runtime_archive_inventory.json"
);
...
.setSingleShot(true)
.setId("JS#SingBoxHubRuntimeArchiveInventory")
.build();
...
var escapedKey = String(key).replace(
/[.*+?^${}()|[\]\\]/g,
"\\$&"
);
...
var inspection = status ?
status.runtimeJarStaticInspection : null;
if (!inspection) {
return null;
}
return {
...
socketName: inspection.socketNameDeclared === true ?
"declared_socket_name" : "",
endpointSize: "",
endpointMtime: String(
...
"  fi",
"  printf 'jarSha\\t%s\\n' \"$SHA_VALUE\"",
"  MAGIC=\"$($TOYBOX od -An -tx1 -N16 \"$JAR\" 2>/dev/null | $TOYBOX tr -d ' \\r\\n')\"",
"  printf 'magicHex\\t%s\\n' \"$MAGIC\"",
"else",
"  printf 'jarExists\\t0\\n'",
"fi",
...
"ENTRY_TOTAL=0",
"ENTRY_TRUNCATED=0",
"if [ -x /system/bin/unzip ] && /system/bin/unzip -Z1 \"$JAR\" >/dev/null 2>&1; then",
"  ARCHIVE_TOOL=system_unzip_z1",
"  ENTRY_TOTAL=\"$(/system/bin/unzip -Z1 \"$JAR\" 2>/dev/null | $TOYBOX wc -l)\"",
"  ENTRY_LIST=\"$(/system/bin/unzip -Z1 \"$JAR\" 2>/dev/null | $TOYBOX head -n " + MAX_ENTRIES + ")\"",
"elif [ -x /system/bin/unzip ] && /system/bin/unzip -l \"$JAR\" >/dev/null 2>&1; then",
"  ARCHIVE_TOOL=system_unzip_l",
"  ENTRY_TOTAL=\"$(/system/bin/unzip -l \"$JAR\" 2>/dev/null | $TOYBOX awk 'NR>3 && $1 ~ /^[0-9]+$/ {n++} END {print n+0}')\"",
"  ENTRY_LIST=\"$(/system/bin/unzip -l \"$JAR\" 2>/dev/null | $TOYBOX awk 'NR>3 && $1 ~ /^[0-9]+$/ {print $4}' | $TOYBOX head -n " + MAX_ENTRIES + ")\"",
"elif $TOYBOX unzip -l \"$JAR\" >/dev/null 2>&1; then",
"  ARCHIVE_TOOL=toybox_unzip_l",
"  ENTRY_TOTAL=\"$($TOYBOX unzip -l \"$JAR\" 2>/dev/null | $TOYBOX awk 'NR>3 && $1 ~ /^[0-9]+$/ {n++} END {print n+0}')\"",
"  ENTRY_LIST=\"$($TOYBOX unzip -l \"$JAR\" 2>/dev/null | $TOYBOX awk 'NR>3 && $1 ~ /^[0-9]+$/ {print $4}' | $TOYBOX head -n " + MAX_ENTRIES + ")\"",
"fi",
"if [ -n \"$ENTRY_TOTAL\" ] && [ \"$ENTRY_TOTAL\" -gt " + MAX_ENTRIES + " ]; then ENTRY_TRUNCATED=1; fi",
"printf 'archiveTool\\t%s\\n' \"$ARCHIVE_TOOL\"",
"printf 'entryTotal\\t%s\\n' \"$ENTRY_TOTAL\"",
"printf 'entryTruncated\\t%s\\n' \"$ENTRY_TRUNCATED\"",
"ENTRY_DATA=\"$(printf '%s' \"$ENTRY_LIST\" | $TOYBOX base64 2>/dev/null | $TOYBOX tr -d '\\r\\n')\"",
"printf 'entryData\\t%s\\n' \"$ENTRY_DATA\"",
"MANIFEST=",
"if [ -n \"$ARCHIVE_TOOL\" ]; then",
"  if [ -x /system/bin/unzip ]; then",
"    MANIFEST=\"$(/system/bin/unzip -p \"$JAR\" META-INF/MANIFEST.MF 2>/dev/null | $TOYBOX head -c " + MAX_MANIFEST_BYTES + ")\"",
"  else",
"    MANIFEST=\"$($TOYBOX unzip -p \"$JAR\" META-INF/MANIFEST.MF 2>/dev/null | $TOYBOX head -c " + MAX_MANIFEST_BYTES + ")\"",
"  fi",
"fi",
"MANIFEST_DATA=\"$(printf '%s' \"$MANIFEST\" | $TOYBOX base64 2>/dev/null | $TOYBOX tr -d '\\r\\n')\"",
"printf 'manifestData\\t%s\\n' \"$MANIFEST_DATA\""
];
...
if (/(^|\/)classes([0-9]+)?\.dex$/.test(lower) || /\.dex$/.test(lower)) {
return "dex";
}
if (/\.class$/.test(lower)) {
return "classFile";
...
function parseEntries(text, inventory) {
var lines = String(text || "").split(/\r?\n/);
var i;
var name;
var type;
for (i = 0; i < lines.length &&
inventory.archiveEntries.length < MAX_ENTRIES; i += 1) {
name = safeEntryName(lines[i]);
if (!name) {
continue;
}
type = entryType(name);
inventory.archiveEntries.push({name: name, type: type});
inventory.entryTypeCounts[type] =
Number(inventory.entryTypeCounts[type] || 0) + 1;
...
inventory.entryTypeCounts.manifest > 0;
}
...
if (safeKeys[key] === true && value.length <= 160 &&
!/[\r\n\t\0]/.test(value)) {
...
if (value.indexOf("504b0304") === 0 ||
value.indexOf("504b0506") === 0 ||
value.indexOf("504b0708") === 0) {
return "zip_archive";
}
...
counts.json + counts.properties + counts.other +
counts.directory + counts.manifest ===
inventory.archiveEntries.length) {
...
plan.runtimeArchiveInventoryState = inventory ?
String(inventory.state || "checking") : "checking";
...
if (map.entryData) {
entriesText = String(new JavaString(
Base64.decode(String(map.entryData), Base64.DEFAULT),
"UTF-8"
));
}
if (map.manifestData) {
manifestText = String(new JavaString(
Base64.decode(String(map.manifestData), Base64.DEFAULT),
"UTF-8"
));
}
inventory = pendingInventory("non_dex_archive_identified", null);
...
inventory.runtimeJarCanonicalPathMatched =
boolValue(map.jarPathMatched);
inventory.runtimeJarSize = numberValue(map.jarSize, -1);
inventory.runtimeJarSha256Prefix =
String(map.jarSha || "").substring(0, 16);
inventory.fileMagicHex = String(map.magicHex || "").substring(0, 32);
inventory.containerKind = containerKind(inventory.fileMagicHex);
inventory.archiveListingAvailable =
!!String(map.archiveTool || "");
inventory.archiveListingTool = String(map.archiveTool || "");
inventory.archiveEntryCount = numberValue(map.entryTotal, 0);
inventory.archiveListingTruncated = boolValue(map.entryTruncated);
parseEntries(entriesText, inventory);
parseManifest(manifestText, inventory);
inventory.targetClassesExpectedExternal =
inventory.entryTypeCounts.dex === 0 &&
inventory.entryTypeCounts.classFile === 0 &&
validClassName(evidence.clientClass) &&
validClassName(evidence.serverClass);
...
var evidence;
status = status || {};
inventory = inventory || cachedInventory ||
pendingInventory("checking", null);
...
return {
ok: inventory.runtimeJarExists === true &&
...
status.runtimeArchiveInventory) {
result.data.runtimeArchiveInventory =
status.runtimeArchiveInventory;
}
} catch (ignoredResult) {}
return result;
...
try {
status = runtime.status();
inventory = status.runtimeArchiveInventory ||
pendingInventory("checking", null);
result.runtimeArchiveInventory =
String(inventory.state || "checking");
result.runtimeArchiveInventoryDetails = inventory;
if (status.protocolDiscovery) {
result.runtimeProtocolDiscovery = String(
status.protocolDiscovery.state || "checking"
);
result.runtimeProtocolDiscoveryDetails =
status.protocolDiscovery;
}
if (status.protocolAdapterPlan) {
result.runtimeProtocolAdapterPlan = String(
status.protocolAdapterPlan.state || "checking"
);
result.runtimeProtocolAdapterPlanDetails =
status.protocolAdapterPlan;
}
} catch (error) {
inventory = pendingInventory(
"archive_inventory_status_unavailable",
SBH.util.errorText(error)
);
...
if (!cachedInventory ||
Number(cachedInventory.schemaVersion) !== CACHE_SCHEMA) {
