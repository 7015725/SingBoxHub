/*
 * SingBoxHub Runtime inventory through ShortX ShellCommand.
 * Rhino ES5 only.
 *
 * Safety boundary:
 * - fixed id/find/test operations only
 * - no su invocation
 * - no network access
 * - no file content reads
 * - no file creation, modification or deletion
 * - no Runtime controller, process, TUN, route, firewall or DNS operations
 */
(function () {
    "use strict";
    ...
    var PROBE = "runtime_shortx_shell_inventory";
    var PROBE_VERSION = 4;
    var MAX_DEPTH = 5;
    var MAX_ENTRIES = 400;
    ...
    function getContextValue(data, key) {
    ...
    function executeShell(command) {
        var action = ShellCommand.newBuilder()
            ...
            .setSingleShot(true)
            .setId("JS#SingBoxHubRuntimeShortXInventoryV4")
            .build();
        ...
        return {
            out: getContextValue(data, "shellOut"),
            err: getContextValue(data, "shellErr"),
    ...
    function buildCommand(runtimeRoot) {
        ...
        return "/system/bin/toybox timeout " + TIMEOUT_SECONDS +
            " /system/bin/sh -c " + shellQuote(script);
    }
    ...
    function parse(runtimeRoot, shellResult) {
        var rootPath = String(runtimeRoot.getAbsolutePath());
        ...
        for (i = 0; i < lines.length; i += 1) {
            ...
            absolutePath = fields.slice(2).join("\t");
            relative = relativePath(rootPath, absolutePath);
            item = {
                path: relative,
                type: fields[1] === "d" ? "directory" :
                    (fields[1] === "f" ? "file" :
                        (fields[1] === "l" ? "symlink" : "other")),
                candidate: isCandidate(relative)
            };
            entries.push(item);
            if (item.candidate) {
    ...
    function run() {
        ...
        var shellResult = executeShell(buildCommand(runtimeRoot));
        var inventory = parse(runtimeRoot, shellResult);
        ...
        return {
            ...
            shellPurpose: "fixed_id_find_and_test_only",
